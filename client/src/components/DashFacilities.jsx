...
export default function DashFacilities() {
        ...
        const fetchFacilities = async () => {
            try {
                ...
                if (currentUser.isAdmin) {
                    ...
                    res = await fetch(`/api/facility/getFacilities?userId=${currentUser._id}`);
                }
    ...
    const handleDeleteFacility = async () => {
        ...
        try {
            const res = await fetch(`/api/facility/deleteFacility/${facilityIdToDelete}`, {
                method: "DELETE",
            });
            const data = await res.json();
    ...
    return (
      ...
      scrollbar scrollbar-track-slate-100 scrollbar-thumb-slate-300 dark:scrollbar-track-slate-700 dark:scrollbar-thumb-slate-500"
        >
            ...
            ) : currentUser && currentUser.isFacilityOwner && facilities.length > 0 ? (
                <>
                        ...
                        <TableBody className="divide-y">
                            {facilities.map((facility) => (
                                ...
                                >
                                    ...
                                    <TableCell>{facility.sports.map((sport) => sport + " ")}</TableCell>

                                    <TableCell>{facility.amenities.map((amenity) => amenity + " ")}</TableCell>
