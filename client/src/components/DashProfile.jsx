...
export default function DashProfile() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/user/update/${currentUser._id.toString()}`,
        {
          method: "PUT",
          headers: {
  ...
  const handleDeleteUser = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/user/delete/${currentUser._id.toString()}`,
        {
          method: "DELETE",
          "Content-Type": "application/json",
        }
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      >
        ...
        />
        <div
          className="relative w-32 h-32 shadow-md overflow-hidden rounded-full"
        ...
        <div className="flex flex-col gap-5 p-5 w-full md:w-100">
          ...
          {currentUser.isAdmin && (
            <Link to="/dashboard?tab=dash">
              <Button
                type="button"
                className="cursor-pointer bg-gradient-to-r from-green-500 to-teal-500 text-white hover:bg-gradient-to-l focus:ring-green-200 dark:focus:ring-green-800 w-full"
              >
                Admin Dashboard
              </Button>
            </Link>
